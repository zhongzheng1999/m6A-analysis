...
save(plotp,predictedBoxdat,file = 'predictedBoxdat.Rdata')
dat_drug <- do.call(rbind, predictedBoxdat)
dat_drug$drug <- str_split(rownames(dat_drug), '\\.T', simplify = T)[,1]

# dat_drug_avg <- dat_drug %>% group_by(drug,ImmClust) %>%  summarise(mean_value = median(est.ic50))
# 
# dat_drug_avg_min <- dat_drug_avg %>% group_by(drug) %>% top_n(wt = mean_value, 1)

compare_dat <- compare_means(est.ic50~ImmClust,data = dat_drug, group.by = 'drug', method = "kruskal.test")
compare_dat <- compare_dat[compare_dat$p.signif != 'ns',]

compare_dat <- compare_means(est.ic50~ImmClust,data = dat_drug, group.by = 'drug', method = "kruskal.test")
write.csv(compare_dat, 'TCGA_compare_dat.csv')
...
# 适合展示多种药物
p2 <- plot_grid(plotlist=plotp[compare_dat$drug], ncol=5)
p2
ggsave(plot = p2,"multiple_1.pdf", width = 20, height = 16)
